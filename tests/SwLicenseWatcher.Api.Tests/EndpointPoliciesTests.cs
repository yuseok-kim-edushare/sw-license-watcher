...
public class EndpointPoliciesTests
{
    [Fact]
    public void Resolve_limits_snapshots_to_8_mib()
    {
        Assert.Equal(8 * 1024 * 1024, EndpointPolicies.GetRequestBodySize(AgentPaths.InventorySnapshots));
    }

    [Fact]
    public void Resolve_limits_heartbeats_to_64_kib()
    {
        Assert.Equal(64 * 1024, EndpointPolicies.GetRequestBodySize(AgentPaths.Heartbeats));
    }
    ...
    {
        Assert.NotNull(EndpointPolicies.GetRequestBodySize(path));
    }
    ...
    {
        Assert.Null(EndpointPolicies.GetRequestBodySize(path));
    }
}
