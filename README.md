...
- **서버 측 API**
  ...
  - 업데이트: `/api/updates/worker/manifest`, GitHub Release 가져오기 `/api/updates/worker/github`, 캐시된 Worker ZIP `GET /api/updates/worker/package/{version}` (토큰 없음. 0.1.x 클라이언트가 이 주소로 파일을 받음)
